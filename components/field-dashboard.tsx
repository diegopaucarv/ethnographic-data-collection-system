...
const forms = [
  { code: 'ETN', title: 'Ficha de escena etnográfica', description: 'Registra una escena con contexto, participantes y reflexividad.', count: '12 campos', tone: 'bg-primary', order: 1, state: 'Siguiente paso' },
  { code: 'OBS', title: 'Jornada de observación', description: 'Identifica el lugar, horario, posición y actividad principal.', count: '9 campos', tone: 'bg-primary/80', order: 2, state: 'Después' },
  { code: 'REC', title: 'Registro de escenas', description: 'Anota descripciones densas, frases e interpretaciones.', count: '5 columnas', tone: 'bg-primary/65', order: 3, state: 'Después' },
  { code: 'MEM', title: 'Memo analítico', description: 'Cierra la jornada con hallazgos y preguntas emergentes.', count: '8 campos', tone: 'bg-primary/50', order: 4, state: 'Cierre' },
]
      ...
      <div className="mx-auto flex max-w-[1500px]">
        ...
        <main className="min-w-0 flex-1 p-4 sm:p-6 lg:p-10">
          <div className="mx-auto max-w-6xl">
            ...
            <section className="mb-10"><Card className="overflow-hidden border-border/70 shadow-sm"><CardHeader className="border-b bg-secondary/40"><div className="flex items-center justify-between gap-4"><div><CardTitle className="text-xl">Ruta de recolección</CardTitle><CardDescription className="mt-1">Sigue este orden para no olvidar ningún paso.</CardDescription></div><Badge variant="outline">4 pasos</Badge></div></CardHeader><CardContent className="p-4 sm:p-6"><div className="grid gap-3 md:grid-cols-4">{forms.map((form, index) => <div key={form.code} className="relative flex gap-3 md:block"><div className="flex size-10 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-bold text-primary-foreground">{form.order}</div><div className="pt-1 md:pt-3"><p className="text-sm font-semibold">{form.title}</p><p className="mt-1 text-xs text-muted-foreground">{form.state}</p></div>{index < forms.length - 1 && <div className="absolute left-5 top-10 h-5 w-px bg-primary/30 md:left-[calc(100%+0.75rem)] md:top-5 md:h-px md:w-6" />}</div>)}</div></CardContent></Card></section>
            <section><div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-bold">Formularios de campo</h2><p className="mt-1 text-sm text-muted-foreground">Empieza por el paso marcado como siguiente.</p></div><button className="hidden items-center gap-1 text-sm font-medium text-primary sm:flex">Ver todos <ChevronRight className="size-4" /></button></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{forms.map((form) => <Card key={form.code} className="group border-border/70 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"><CardHeader><div className="mb-2 flex items-center justify-between"><div className={`flex size-11 items-center justify-center rounded-xl text-sm font-bold text-primary-foreground ${form.tone}`}>{form.code}</div><Badge variant="secondary">{form.count}</Badge></div><CardTitle className="text-lg leading-snug">{form.title}</CardTitle><CardDescription className="min-h-10 leading-5">{form.description}</CardDescription></CardHeader><CardContent><Button variant={form.order === 1 ? 'default' : 'outline'} className="w-full" onClick={() => { setSelectedForm(form); setShowForm(true) }}>{form.order === 1 ? 'Comenzar aquí' : 'Abrir formulario'} <ChevronRight data-icon="inline-end" /></Button></CardContent></Card>)}</div></section>
